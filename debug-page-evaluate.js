...
async function debugPageEvaluate() {
    console.log('🔍 Debugging Page Evaluate...\n');
    ...
    try {
        ...
        // Debug the exact same logic as in FormScanner
        const debugResult = await page.evaluate((isDev) => {
            const results = [];
            results.push(`isDev: ${isDev}`);
            
            // Get all Qr7Oae elements
            const questionElements = document.querySelectorAll('.Qr7Oae[role="listitem"]');
            results.push(`Found ${questionElements.length} .Qr7Oae[role="listitem"] elements`);
            
            questionElements.forEach((element, index) => {
                results.push(`\n=== Element ${index + 1} ===`);
                
                // Test the exact same text extraction logic
...
debugPageEvaluate().catch(console.error);
